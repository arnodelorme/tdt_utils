...
xx = sin(pnts)*radius;
yy = cos(pnts)*radius;
plot3( x, y, ones(size(x))*max(values)*1.5, 'k.');
...
plot3(earx,eary,ones(size(earx)),'color','k','LineWidth',2)    % plot left ear
plot3(-earx,eary,ones(size(earx)),'color','k','LineWidth',2)   % plot right ear

nosex = [0.0900 0.0400 0.010 0 -0.010 -0.0400 -0.0900];
nosey = [0.492  0.5300 0.5700 0.570 0.5700 0.5300 0.492 ];
plot3(nosex, nosey, ones(size(nosex)),'color','k','LineWidth',2)   % plot right ear
axis equal
