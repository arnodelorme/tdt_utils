...
%
% Author: Arnaud Delorme, 2006 for Robert Lawson
...
while cont
   ...
   elseif ~isnan(str2double(firstelem)), cont = 0;
   end;
   count = count+1;
end;
warning on;
...
if count == 0, 
    chans = cell(1,nc);
    minnc = min(nc, length(chanlist));
    chans(1:minnc) = chanlist(1:minnc);
    for index = minnc+1:nc
        chans{index} = sprintf('Unknown%d', index);
    end;
    res = 1;
    return; 
end;
