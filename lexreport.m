function lexreport(filein, varargin)
...
if nargin < 1
    ...
    cb_loadbands = [ '[tmpf tmpp] = uigetfile;' ...
                     'if tmpf(1) ~= 0,' ...
                     '    lexreport(gcbf, ''loadbands'', fullfile(tmpp, tmpf));' ...
                     'end' ...
                     'clear tmpf tmpp;' ];
...
% ---------------------------------------------------------------
[a b c]  = intersect(lower(chans), lower(channel_order));
[tmp cc] = sort(c);
neworder = b(cc);
    ...
    switch opt.exportlist{index}
        ...
        case 'FFT Coherence'
            ...
            if strcmpi(opt.exportbands, 'on'), 
                [lexcoh lexstd ] = lexcoherence(tmpdata, srate, banddefs);
                if ~f_sd, lexstd = []; end
...
function str = myloadband(filename)
    ...
    fid = fopen(filename);
    tmp = textscan(fid, '%s', [ 3 Inf ]);
    fclose(fid);
    maxlen = floor(length(tmp{1})/3)*3;
    str = reshape(tmp{1}(1:maxlen), 3, maxlen/3);
    str = str';
    for band = 1:size(str,1)
        str{band,2} = str2num(str{band,2});
        str{band,3} = str2num(str{band,3});
    end
